...
raw_data = df.drop(columns_to_drop, axis=1).dropna()
...
raw_data["Team"] = raw_data["Team"].apply(Clean_team_names)

raw_data["GD"] = raw_data["GF"] - raw_data["GA"]
raw_data["Pts"] = (raw_data["W"] * 3) + (raw_data["D"])

aggregated_data = raw_data.groupby("Team", as_index=False).sum()
sorted_data = aggregated_data.sort_values(["Pts", "GD"], ascending=False)

sorted_data["Pos"] = np.arange(len(sorted_data))
sorted_data["Pos"] = sorted_data["Pos"] + 1
